<?php ob_start(); ?>

<section class="bloc_page2 container-fluid">
	
	<div class="row">
      
    <?php require("backAside.php") ?>

    <div class="admin col-xs-12 col-sm-12 col-md-8  col-lg-9 col-xl-10">

      <div class="row">
        <div class="col-12 text-center">
        	<h2>Tous les commentaires</h2>
        </div>
      </div>
      <div>
        <table class="table table-light table-bordered table-hover table-responsive-lg">
          <thead class="thead-light">
            <tr>
              <th>Chapitre concerné</th>
              <th>Auteur</th>
              <th class="publication_date">Date de publication</th>
              <th>Voir le commentaire</th>
              <th>Supprimer</th>
            </tr>
          </thead>
          <tbody>
            <?php 
            while ($comment = $comments->fetch())
            {
            ?>
            <tr class="<?php if ($comment['status'] == 'signalé') echo 'warning';  ?>">
              <th>
              <i class="fas fa-bookmark"></i>
              <?php echo htmlspecialchars($comment['title']); ?>
              </th>                 
              <th>
              <?php echo htmlspecialchars($comment['author']); ?>
              </th>
              <th class="publication_date publication_date_fr">
              le <?php echo htmlspecialchars($comment['comment_date_fr']); ?>	
              </th>

              <th>
              <a href="index.php?action=updateAComment&amp;id=<?= $comment['id'] ?>" class="btn btn-outline-primary btn-post" role="button">
              	<i class="fas fa-edit"></i>
              </a>
              </th>
              <th>
              <a href="index.php?action=deleteAComment&amp;id=<?= $comment['id'] ?>" class="btn btn-outline-primary btn-post" role="button">
              	<i class="fas fa-trash-alt"></i>
              </a>        
              </th>
            </tr>	
            <?php 
            }   
            $comments->closeCursor();
            ?>
          </tbody>
        </table>
			</div>		
		</div>
	</div>
</section>

<?php $content = ob_get_clean(); ?>

<?php require('backTemplate.php'); ?>